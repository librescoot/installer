...
void main() {
  test('disarm command is POSIX syntax and safe to retry', () async {
    ...
    expect(
      command,
      contains('a legacy installer onboot script is still armed'),
    );
    ...
    expect(
      command,
      contains('systemctl stop librescoot-onboot-inline librescoot-onboot'),
    );
    expect(command, contains(r"pkill -f '^/bin/sh /data/onboot\.sh'"));
    expect(
      command,
      contains(r"pkill -f '/data/installer/scripts/[0-9][0-9]-'"),
    );
  ...
  test('keycard prestart guards master mode before service start', () async {
    final command = SshService.keycardPrestartCommand;
    final directory = await Directory.systemTemp.createTemp(
      'keycard-prestart-',
    );
    addTearDown(() => directory.delete(recursive: true));
    final script = File('${directory.path}/prestart.sh');
    await script.writeAsString(command);

    final syntax = await Process.run('sh', ['-n', script.path]);
    expect(syntax.exitCode, 0, reason: syntax.stderr.toString());
    final guard = command.indexOf(
      'redis-cli lpush scooter:keycard learn:master:stop',
    );
    final start = command.indexOf('systemctl start "\$unit"');
    expect(guard, greaterThan(-1));
    expect(start, greaterThan(guard));
    expect(command, contains('LoadState'));
  });
  ...
  test('service recovery verifies masks and required active units', () async {
    ...
    final start = command.indexOf('systemctl start librescoot-keycard');
    expect(guard, greaterThan(-1));
    expect(start, greaterThan(guard));
  });
}
