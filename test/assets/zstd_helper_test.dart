...
void main() {
  ...
  test('the unpack runs zstd by name on the dashboard, not an empty word', () {
    // The heredoc that writes this block is unquoted, so an unescaped
    // $ZSTD_BIN resolved on the laptop, to nothing, and the dashboard was
    // asked to run "-d -f -o ...". Every routing tile install failed with
    // "sh: -d: not found" and the phase never completed.
    expect(template, contains(r'\$ZSTD_BIN -d -f -o'),
        reason: 'ZSTD_BIN must survive rendering and resolve on the device');
    expect(template, isNot(contains(r'"$ZSTD_BIN -d')),
        reason: 'an unescaped use renders as an empty command');
  });
}
