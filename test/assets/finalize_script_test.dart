...
void main() {
  ...
  group('rendering', () {
    ...
    test('every placeholder the template carries is one we fill', () {
      expect(
        FinalizeScript.unresolvedPlaceholders(render()),
        isEmpty,
  ...
  group('running it', () {
    ...
    // [nrfStatus] is what successive reads of ble[firmware-update-status]
    // answer; the last entry repeats once the list runs out. An empty string
    // is the field being absent, which is what a bluetooth-service that has
    // not heard from the nRF yet looks like.
    Future<void> stubs({
      ...
      final nrfCases = [
        for (var i = 0; i < nrfStatus.length - 1; i++)
          '${i + 1}) echo "${nrfStatus[i]}" ;;',
        '*) echo "${nrfStatus.last}" ;;',
      ].join('\n      ');
      final entries = {
        'redis-cli':
...
case "\$*" in
  ...
  *"hget ble firmware-update-status"*)
    n=\$(cat "\$CALLS.nrf" 2>/dev/null || echo 0)
    n=\$((n + 1))
    echo "\$n" > "\$CALLS.nrf"
    case "\$n" in
      $nrfCases
...
''',
    ...
    Future<ProcessResult> run({
      ...
      if (status != null) {
        await File(
          '${root.path}/installer/trampoline-status',
      ...
      if (previousRoot != null) {
        await File(
          '${root.path}/installer/previous-root',
    ...
    test('an upgrade gets the pre-install settings back', () async {
      await Directory('${root.path}/installer').create(recursive: true);
      await File(
        '${root.path}/settings.toml.preinstall',
    ...
    test('a good run unlocks and records what it installed', () async {
      ...
      expect(
        await File('${root.path}/installer/last-install').readAsString(),
        contains('run-id: run-good'),
      );
    });

    test(
      'a good run ends with the bar complete and the vehicle dark',
      () async {
        ...
        expect(
          log,
          contains('unit librescoot-bootled-guard'),
          reason: 'amber has to be held while the finish runs',
        );
        expect(
          log.any((l) => l.contains('ioctl /dev/pwm_led6 0x0000754A -v 150')),
          isTrue,
          reason: 'segment 4 never filled',
        );
        ...
        expect(
          log.indexOf('systemctl stop librescoot-bootled-guard.service'),
        ...
        expect(
          File('${root.path}/installer/trampoline-phase').existsSync(),
          isFalse,
        );
      },
    );

    group('the nRF firmware update', () {
      int statusReads(List<String> log) => log
          .where((l) => l.contains('hget ble firmware-update-status'))
          .length;

      test('holds the unlock, and the bar, until the flash is over', () async {
        // bluetooth-service flashes the nRF52 on the first boot into the new
        // image, and an owner who sees the unlock and rides off mid-flash is
        // the case this exists for. Busy states are checking, updating and
        // recovering; the first terminal one releases the wait.
        final result = await run(
          status: 'success',
          nrfStatus: const ['', 'checking', 'updating', 'updating', 'success'],
        );
        expect(result.exitCode, 0, reason: result.stderr.toString());
        final log = await callLog();
        expect(statusReads(log), 5);
        final lastRead = log.lastIndexWhere(
          (l) => l.contains('hget ble firmware-update-status'),
        );
        expect(
          lastRead,
          lessThan(log.indexWhere((l) => l.contains('scooter:state unlock'))),
        );
        // Segment 4 keeps breathing and the LED stays amber while the flash
        // runs: a vehicle gone dark and still locked reads as a failed handover.
        // lastIndexOf: starting the guard stops a stale one first, so the
        // stop that matters is the last one.
        expect(
          lastRead,
          lessThan(
            log.lastIndexOf('systemctl stop librescoot-bootled-guard.service'),
          ),
        );
        expect(
          lastRead,
          lessThan(
            log.lastIndexWhere(
              (l) => l.contains('ioctl /dev/pwm_led6 0x0000754A -v 150'),
            ),
          ),
        );
      });

      test('gives up on a status that never appears', () async {
        // Nothing is written until the nRF has reported its version, and a
        // dead nRF never does. The unlock does not need it.
        final result = await run(status: 'success', nrfStatus: const ['']);
        expect(result.exitCode, 0, reason: result.stderr.toString());
        final log = await callLog();
        // One read a second from 0 to 60: the field has the whole grace
        // period to appear, and the read at its end is the one that gives up.
        expect(statusReads(log), 61);
        expect(log.any((l) => l.contains('scooter:state unlock')), isTrue);
      });

      test('does not leave the vehicle locked behind a hung flash', () async {
        final result = await run(
          status: 'success',
      ...
      test('a failed run does not wait for it', () async {
        final result = await run(
          status: 'error: something',
          nrfStatus: const ['updating'],
        );
        expect(result.exitCode, 0, reason: result.stderr.toString());
        expect(statusReads(await callLog()), 0);
      });
    });
    ...
    test('a failed run stays reachable so it can be retried', () async {
      ...
      await File(
        '${root.path}/service-mode.json',
      ).writeAsString('{"active":true,"name":"service"}');
      final result = await run(
        status: 'error: dbc never answered',
      ...
      expect(
        log.any((l) => l.contains('scooter:state unlock')),
        isFalse,
    ...
    test('a good run does end it, which is what takes the link down', () async {
      ...
      final result = await run(status: 'success', serviceModeActive: 'false');
      expect(result.exitCode, 0, reason: result.stderr.toString());
      final log = await callLog();
    ...
    group('what it does when the install did not land', () {
      test('a rolled-back board is not unlocked', () async {
        ...
        expect(
          await callLog(),
          isNot(contains(contains('scooter:state unlock'))),
        );
      });

      test(
        'a bootstrap failure publishes a terminal status for the laptop',
        () async {
          await run(
            status: null,
      ...
      test(
        'it stays queued when it declines, so a retry has a handover',
        () async {
      ...
      test(
        'a different slot is the installed image, and the marker goes',
        () async {
          await run(
            status: 'success',
      ...
      test(
        'no marker means no reboot happened, and no verdict either',
        () async {
