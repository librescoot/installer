...
/// A widget whose build returns a [PhaseLayout]. The phase host has to know
/// this before building it: a PhaseLayout owns its own scrolling and action
/// bar and needs the height, and wrapping one in the host's scroll view puts
/// its Expanded body under unbounded constraints, which throws instead of
/// rendering. The host recognises a bare PhaseLayout by type; a widget that
/// merely returns one is invisible to that check unless it carries this.
mixin OwnsPhaseLayout on Widget {}
        ...
        if (actions.isNotEmpty || onBack != null)
          Container(
            ...
            child: Center(
              child: ConstrainedBox(
                constraints: const BoxConstraints(maxWidth: _measure),
                // A Row of fixed-width buttons overflows once a stage needs
                // four of them at once; Wrap lets a crowded bar grow to a
                ...
                child: Row(
                  children: [
                    ...
                    if (leaving.isNotEmpty)
                      // flex 0: the left slot takes what it needs and no more,
                      // so the slack collects in the middle and the onward
                      // group stays flush right instead of splitting the row.
                      Flexible(
                        flex: 0,
                        child: Wrap(
                          spacing: 8,
