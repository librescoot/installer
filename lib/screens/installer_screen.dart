...
class _InstallerScreenState extends State<InstallerScreen> with WindowListener {
  ...
  /// What the current connect attempt ran into, with the raw exception under
  /// it. Null while an attempt is running without having hit anything.
  ConnectFailure? _connectFailure;

  /// Whether a USB device has turned up during this attempt. A board that was
  /// never on the bus and one that walked away mid-connect are different
  /// problems with different answers, and the exception says neither.
  bool _usbDeviceSeen = false;

  /// How long the connect phase waits for a USB device before it starts
  /// saying what to check. Long enough that a board which is merely slow to
  /// enumerate never gets there.
  static const Duration _usbWaitBeforeAdvice = Duration(seconds: 30);
  ...
  bool get _mdbConnectNoRoute =>
      _connectFailure?.kind == ConnectFailureKind.localNetworkBlocked;
  Timer? _noRouteRetry;

  DateTime? _noRouteRetryStart;

  bool get _noRouteWatchSpent {
    final since = _noRouteRetryStart;
    return since != null &&
        DateTime.now().difference(since) > _noRouteRetryWindow;
  }
  ...
  bool _mdbFlashOwnershipUncertain = false;
  ...
  bool _bluetoothPairingStarting = false;
  int _bluetoothPairingGeneration = 0;
  String? _blePinCode;
  ...
  bool _keycardLearningStarting = false;
  int _keycardLearningGeneration = 0;
  ...
  bool _finishCompletionExhausted = false;
  ...
  void _setPhase(InstallerPhase phase) {
    ...
    if (phase == InstallerPhase.cbbReconnect) {
      ...
      _cbbAutoCheckStarted = false;
    } else if (leaving == InstallerPhase.cbbReconnect) {
      _cbbPollAbandoned = true;
    }
    if (leaving == InstallerPhase.mdbConnect && phase != leaving) {
      _noRouteRetry?.cancel();
  ...
  Future<void> _startFinishEntry() async {
    if (!_finishAttempt.isRunning) _finishAttempt.reset();
    final generation = _finishAttempt.begin();
    if (generation == null) {
      debugPrint('UI: finish entry already running, not starting another');
      return;
    }
    final criticalOperation = _acquireCriticalOperation();
    try {
      await _onEnterFinish();
      _finishAttempt.complete(generation);
    } catch (e, stack) {
      _finishAttempt.fail(generation, e);
      debugPrint('UI: finish entry failed: $e\n$stack');
      if (mounted) {
        final failureState = finishEntryFailureState();
        setState(() {
          _finishBlocked = failureState.blocked;
          _awaitingFinishHandover = failureState.awaitingHandover;
        });
      }
    } finally {
  ...
  Future<void> _onEnterFinish() async {
    ...
    // Owed on paper, but the device may be mid-way through it: the
    // coordinator running, or phases still queued for a boot. Re-arming on
    // top of that stages a second copy of the artifact phase over one that
    // may be writing, wipes the completion record, and starts a second
    ...
    try {
      ...
      await _sshService.runCommand(
        'mkdir -p ${SshService.installerScriptsDir}',
      );
      ...
      debugPrint(
        'UI: staged ${MdbArtifactScript.phaseName} and '
  ...
  Future<void> _refreshFinishCompletion() async {
    if (_isDryRun ||
        _finishCompletionConfirmed ||
        _finishCompletionChecking ||
  ...
  Future<void> _copyToClipboard(String text) async {
    ...
    try {
      ...
      await proc.exitCode.timeout(
        ...
        onTimeout: () {
          proc.kill();
          throw TimeoutException('pbcopy did not finish');
        },
      );
    } catch (e) {
                  ...
                  child: Row(
                    children: [
                      PhaseSidebar(
                        ...
                        statusMessage:
                            _currentPhase == InstallerPhase.finish &&
  ...
  Widget _phaseContent(AppLocalizations l10n) {
    final blocked = _driverBlocked;
    if (blocked != null) return _buildDriverBlocked(l10n, blocked);
    ...
    return PhaseLayout(
      ...
      child: Column(
        ...
        children: [
          ...
          Builder(
            builder: (context) {
              final (lead, dos, why) = NoticeCard.splitBullets(
          ...
          Builder(
            builder: (context) {
              final (lead, bullets, _) = NoticeCard.splitBullets(
  ...
  Future<void> _kickoffDownloads() async {
    ...
    try {
      if (launchArgs.hasLocalImagesOnly) {
        ...
        if (!_ownsDownloadGeneration(generation, cancellationToken)) return;
        // Covers everything buildDownloadQueue does under the hood: the tile
        // repo round trips when maps are wanted, plus assembling the queue
        // itself. Set unconditionally so the status line never sits on a
        // step that already finished for however long this takes.
        _setStatus(l10n.preparingDownloads);
        ...
        // image only. The rest of the queue, the artifacts and the tiles, is
        // still the release's, which is what makes a locally built stage-0
        // usable in an otherwise ordinary install.
        for (final entry in {
          DownloadItemType.mdbFirmware: (
            launchArgs.mdbImage,
          ...
          final (path, bmapType) = entry.value;
          if (path == null) continue;
          final at = items.indexWhere((i) => i.type == entry.key);
          if (at < 0) continue;
          items[at] = await _localImageItem(entry.key, path);
          debugPrint('Downloads: ${entry.key} overridden with $path');

          // A bmap describes the block layout of one specific image, so the
          // release's cannot be paired with a local one: the write would skip
          // whatever the two disagree about and report success. Take the
          // image's own if it is beside it, and otherwise drop the bmap so the
          // flasher writes the image whole.
          final bmapAt = items.indexWhere((i) => i.type == bmapType);
          if (bmapAt < 0) continue;
          final sibling = File(
            path.replaceFirst(RegExp(r'\.sdimg\.gz$'), '.sdimg.bmap'),
          );
          if (path.endsWith('.sdimg.gz') && await sibling.exists()) {
            items[bmapAt] = await _localImageItem(bmapType, sibling.path);
            debugPrint('Downloads: $bmapType overridden with ${sibling.path}');
          } else {
            items.removeAt(bmapAt);
            debugPrint('Downloads: $bmapType dropped, no bmap beside $path');
          }
        }
        setState(() {
  ...
  void _downloadInBackground(
    ...
    // The resolve this follows takes milliseconds when the manifest is
    // cached, so leaving its status up meant the sidebar said "resolving
    // releases" for the several minutes the transfer actually takes. Reuses
    // the string the Continue button already shows, so the two agree.
    if (_ownsDownloadGeneration(generation, cancellationToken) && mounted) {
      _setStatus(AppLocalizations.of(context)!.noticesWaitingForDownloads);
    }
    try {
      ...
      if (_ownsDownloadGeneration(generation, cancellationToken)) {
        setState(() {});
        if (mounted) {
          _setStatus(AppLocalizations.of(context)!.downloadsFinished);
        }
      }
    } on DownloadCancelled {
      return;
  ...
  Future<bool> _ensureDriverBinding() async {
    ...
    if (diagnosis.state == DriverBinding.correct ||
      ...
      if (_driverBlocked != null && mounted) {
        setState(() => _driverBlocked = null);
      }
      return true;
    }

    debugPrint(
      'Driver: binding drifted to ${diagnosis.state.name}, '
    ...
    return DriverBlockedPanel(
      ...
      actions: [
        PhaseAction(
          ...
          onPressed: () async {
            setState(() => _driverBlocked = null);
            final ok = await _ensureDriverBinding();
            if (ok && mounted && _currentPhase == InstallerPhase.mdbConnect) {
              _retryMdbConnect();
            }
          },
        ),
  ...
  Widget _buildMdbConnect(AppLocalizations l10n) {
    ...
    // Ahead of the wait rather than after it: the case where no USB device
    // ever appears keeps polling the bus while it shows its advice, so it is
    // a failure with the attempt still running behind it.
    final failure = _connectFailure;
    if (failure != null) return _buildConnectFailure(l10n, failure);
      ...
      return PhaseLayout(
        ...
        actions: [
          PhaseAction(
            ...
            primary: true,
            onPressed: () {
              setState(() => _rootPasswordUnknown = false);
              _retryMdbConnect();
            },
          ),
        ],
  ...
  Widget _buildConnectFailure(AppLocalizations l10n, ConnectFailure failure) {
    return ConnectFailurePanel(
      ...
      body: failure.kind.body(l10n),
      checklistTitle: l10n.connectFailedWhatToCheck,
      detailsLabel: l10n.connectFailedDetailsLabel,
      details: failure.details,
      copyLabel: l10n.copyToClipboard,
      askForHelpLabel: l10n.openLibrescootDiscord,
      onAskForHelp: () => _openExternalUrl(discordUrl),
      actions: [
        ...
        if (failure.kind == ConnectFailureKind.localNetworkBlocked) ...[
          PhaseAction(
            label: l10n.macosOpenLocalNetworkSettings,
            icon: Icons.settings,
            primary: !_noRouteWatchSpent,
            onPressed: _openLocalNetworkSettings,
          ),
          if (_noRouteWatchSpent)
            PhaseAction(
              label: l10n.retryMdbConnect,
              icon: Icons.refresh,
              primary: true,
        ...
        // The self-healing ones are still watching, and say so. A retry there
        // would restart an attempt that never stopped.
        else if (!failure.selfHealing)
          PhaseAction(
            label: l10n.retryMdbConnect,
            icon: Icons.refresh,
            primary: true,
  ...
  void _retryMdbConnect() {
    setState(() {
      _connectFailure = null;
      _mdbConnectStarted = true;
    });
    _noRouteRetryStart = null;
    NetworkService.allowElevationPromptAgain();
    Future.microtask(_autoConnectMdb);
  }

  /// Name what went wrong, and write down enough of it that a bug report has
  /// something to go on. [error] is null for the one failure that never
  /// throws, the wait for a USB device that nothing answers.
  ConnectFailure _describeConnectFailure(Object? error) {
    final device = _device;
    final kind = classifyConnectFailure(
      error: error,
      platform: Platform.operatingSystem,
      usbDevicePresent: device != null,
      usbDeviceSeen: _usbDeviceSeen,
    );
    return ConnectFailure(
      kind: kind,
      details: describeConnectFailureForSupport(
        kind: kind,
        error: error,
        platform: Platform.operatingSystem,
  ...
  static const Duration _noRouteRetryWindow = Duration(minutes: 10);
  ...
  void _scheduleNoRouteRetry() {
    final since = _noRouteRetryStart ??= DateTime.now();
    if (DateTime.now().difference(since) > _noRouteRetryWindow) {
      debugPrint(
        'UI: local network still blocked after '
        '${_noRouteRetryWindow.inMinutes} minutes, leaving the panel up',
      );
      return;
    }
    _noRouteRetry?.cancel();
  ...
  void _userCancelUnlockWait() {
    if (_unlockCompleter != null && !_unlockCompleter!.isCompleted) {
      ...
      _unlockCancelledFromRtd = _awaitingUnlockState == 'ready-to-drive';
      _unlockCompleter!.complete(false);
    }
  }
  ...
  Future<void> _autoConnectMdb() async {
    final l10n = AppLocalizations.of(context)!;
    final keepFailure = _mdbConnectNoRoute;
    setState(() {
      _isProcessing = true;
      if (!keepFailure) _connectFailure = null;
      _usbDeviceSeen = false;
    });
    ...
    // Wait for any USB device (RNDIS or UMS). The wait itself has no end: a
    // board that is slow to enumerate is ordinary and the poll picks it up
    // whenever it arrives. What it grows after a while is advice, because a
    // spinner that has said "waiting for USB device" for a minute tells
    // someone whose cable is in the wrong port nothing they can act on.
    final adviseAt = DateTime.now().add(_usbWaitBeforeAdvice);
    while (_device == null) {
      if (_connectFailure == null && DateTime.now().isAfter(adviseAt)) {
        setState(() => _connectFailure = _describeConnectFailure(null));
      }
      await Future.delayed(const Duration(seconds: 1));
      if (!mounted) return;
    }
    // It turned up on its own, so the advice goes and the wait carries on
    if (_connectFailure != null && !keepFailure) {
      setState(() => _connectFailure = null);
    }
    _usbDeviceSeen = true;
    ...
    try {
      ...
      try {
        ...
        if (leftoversPresent) {
          ...
          // A finish on the device sweeps the status file and leaves the
          // trampoline behind, so the files alone read as a run that never
          // reached a verdict. The completion record is that verdict.
          if (written == TrampolineResult.running &&
              await _sshService.lastInstallSucceeded()) {
            written = TrampolineResult.success;
          }
        }
      ...
      if (!ok) {
        if (mounted) {
          _setStatus(
            _unlockCancelledFromRtd
                ? l10n.parkWaitCancelled
                : l10n.unlockWaitCancelled,
          );
          setState(() => _isProcessing = false);
        }
        return;
      ...
      if (NetworkService.isNoRouteToHost(e)) {
        final iface = await NetworkService().findLibrescootInterface();
      ...
      setState(() {
        _isProcessing = false;
        // Not knowing the root password already has a screen, and where to
        // find one is a better answer than which login was rejected.
        if (!_rootPasswordUnknown) {
          _connectFailure = _describeConnectFailure(e);
        }
      });
  ...
  String _resumeReport(AppLocalizations l10n) {
    final b = StringBuffer()
      ..writeln(
        'Librescoot Installer $appVersion (${Platform.operatingSystem})',
      )
      ..writeln('Interrupted installation');
    if (_resumeStage != null) {
      final actor = _resumeActor == null ? '' : ' [$_resumeActor]';
      b.writeln(
        'Stage: ${_localizedStage(_resumeStage!, l10n)}'
        ' ($_resumeStage)$actor',
      );
    }
    final info = _mdbInfo;
    ...
    if (_resumeLogTail.isNotEmpty) {
      b
        ...
        ..writeln('Scooter log:')
        ..writeln('```')
        ..writeln(_resumeLogTail.trimRight())
        ..writeln('```');
    }
    b
      ..writeln()
      ..writeln('Installer log:')
      ..writeln('```')
          ...
          if (_resumeStage != null)
            Row(
              children: [
                ...
                Flexible(
                  child: Text(
                    l10n.resumeStageLabel(_localizedStage(_resumeStage!, l10n)),
  ...
  }) => _waitForAnyDevice([mode], timeout: timeout);

  /// Waits for the board to turn up in any of [modes], so a caller that can
  /// use more than one does not have to pick wrong and time out against a
  /// board that is right there.
  Future<bool> _waitForAnyDevice(
    ...
    while (!modes.contains(_device?.mode)) {
      if (DateTime.now().isAfter(deadline)) return false;
      // A board that came back under another driver never reaches any of
      // these modes on its own. Put ours back once, the same as the RNDIS
      // wait does, rather than sit out the deadline against it.
      if (!repairAttempted && _device?.mode == DeviceMode.hijacked) {
        repairAttempted = true;
        debugPrint('Wait: device held by another driver, repairing binding');
        await _ensureDriverBinding();
      }
      await Future.delayed(const Duration(seconds: 1));
  ...
  Future<bool> _waitForRndisWithTimeout(
    ...
    while (_device?.mode != DeviceMode.ethernet) {
      ...
      if (elapsed >= timeout) {
        debugPrint(
          'Reconnect: RNDIS never came back within '
            ...
            warning(
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  TextButton.icon(
                    onPressed: () => _openExternalUrl(discordUrl),
                    icon: const Icon(
                      Icons.chat_bubble_outline,
                      size: 16,
                      color: Colors.amber,
  ...
  /// the sidebar's spinner doing the spinning, and whatever they can reach
  /// for if it stalls.
  ...
  Future<void> _deactivateMainBattery() async {
    ...
    if (_mdbLacksLibrescootStack) {
      debugPrint(
        'Battery: no Librescoot stack, skipping main-pack deactivation',
      );
      return;
    }
    try {
  ...
  Future<void> _configureMdbUms(int generation) async {
    ...
    _beginWait([
      if (resuming)
        WaitStep(
          label: l10n.waitingForMdb,
          typical: const Duration(seconds: 45),
        ),
      WaitStep(
      ...
      WaitStep(
        label: l10n.rebootingMdbUms,
        typical: const Duration(seconds: 60),
      ),
      WaitStep(
    ...
    try {
      if (resuming) {
        // The board rebooted out from under the previous attempt. A previous
        // attempt that got as far as writing the bootloader leaves bootcmd on
        // `ums`, so it comes back in mass storage rather than ethernet, which
        // is the state the next phase wants anyway.
        _setStatus(l10n.waitingForMdb);
    ...
    return PhaseLayout(
      ...
      actions: [
        if (_mdbFlashBlocked) ...[
          ...
          if (!_mdbFlashOwnershipUncertain)
            PhaseAction(
  ...
  void _blockMdbFlash({bool ownershipUncertain = false}) {
    if (!mounted) return;
    setState(() {
      _isProcessing = false;
      _mdbFlashStarted = false;
      _mdbFlashBlocked = true;
      _mdbFlashOwnershipUncertain =
          _mdbFlashOwnershipUncertain || ownershipUncertain;
    });
  }

  Future<void> _flashMdb() async {
    if (_windowClosing || !mounted) return;
    var retryFlashAfterCleanup = false;
    final l10n = AppLocalizations.of(context)!;
    ...
    _beginWait([
      ...
      WaitStep(label: l10n.flashingMdb, typical: const Duration(seconds: 90)),
    ]);
    setState(() => _isProcessing = true);
    ...
    try {
      ...
      _setStatus('$diagnosis\n\n${l10n.waitingForDeviceRedetection}');
      final back = await _waitForMassStorageDevice(
        timeout: const Duration(seconds: 60),
        afterDeviceGeneration: failureDeviceGeneration,
      );
      ...
      try {
        if (!_mdbFlashOwnershipUncertain) {
          ...
          debugPrint(
            'Flash: keeping host disk suppression because the writer may '
      ...
      if (retryFlashAfterCleanup && mounted) {
        setState(() => _mdbFlashStarted = false);
      }
    }
  }

  Future<bool> _waitForMassStorageDevice({
    required Duration timeout,
    required int afterDeviceGeneration,
  }) async {
    final deadline = DateTime.now().add(timeout);
    while (DateTime.now().isBefore(deadline)) {
      if (!mounted) return false;
      if (_usbDetector.deviceEventGeneration <= afterDeviceGeneration) {
        await Future.delayed(const Duration(seconds: 1));
        continue;
      }
      final detected = _usbDetector.currentDevice;
      if (detected == null || detected.mode != DeviceMode.massStorage) {
        await Future.delayed(const Duration(seconds: 1));
        continue;
      }
      final detectedIdentity = (
        detected.id,
        detected.mode,
        detected.path,
        detected.sizeBytes,
      ...
      final current = _usbDetector.currentDevice;
      ...
      if (identityStillMatches &&
          UsbDetector.acceptsFreshMassStorageTarget(
            failureGeneration: afterDeviceGeneration,
            currentGeneration: _usbDetector.deviceEventGeneration,
            device: current,
            path: path,
          )) {
        _device = current;
  ...
  static const Duration _mdbBootCeiling = Duration(minutes: 10);

  void _startMdbBoot({bool explicitRetry = false}) {
    if (!mounted || _currentPhase != InstallerPhase.mdbBoot) return;
    if (explicitRetry) NetworkService.allowElevationPromptAgain();
  ...
  Future<void> _waitForMdbBoot(int generation) async {
    ...
    final deadline = DateTime.now().add(_mdbBootCeiling);
    Duration remaining() => deadline.difference(DateTime.now());
    bool expired() => !DateTime.now().isBefore(deadline);
    while (true) {
      ...
      if (expired()) {
        _failMdbBoot(generation, l10n.mdbBootGaveUp(_mdbBootCeiling.inMinutes));
        return;
      }
    ...
    await _ensureDriverBinding();
    if (!_ownsMdbBootAttempt(generation) || expired()) {
      if (expired()) {
        _failMdbBoot(generation, l10n.mdbBootGaveUp(_mdbBootCeiling.inMinutes));
      }
      return;
    ...
    while (stableCount < 10) {
      ...
      final reachable = await _pingMdb(timeout: remaining());
      if (!_ownsMdbBootAttempt(generation)) return;
      if (expired()) {
        _failMdbBoot(generation, l10n.mdbBootGaveUp(_mdbBootCeiling.inMinutes));
        return;
      ...
      final pause = remaining();
      if (pause <= Duration.zero) {
        _failMdbBoot(generation, l10n.mdbBootGaveUp(_mdbBootCeiling.inMinutes));
        return;
      }
      await Future.delayed(
        pause < const Duration(seconds: 1) ? pause : const Duration(seconds: 1),
      );
      if (!_ownsMdbBootAttempt(generation)) return;
    }
  ...
  Set<DownloadItemType> get _requestedHandoffDownloadTypes {
    ...
    if (plan?.installTiles ?? _downloadState.wantsOfflineMaps) {
      types.addAll([DownloadItemType.osmTiles, DownloadItemType.valhallaTiles]);
    }
    return types;
  }
  ...
  Future<bool> _pingMdb({Duration? timeout}) async {
    try {
      final operation = Process.run(
        'ping',
      ...
      actions: [
        ...[
          ...
          PhaseAction(
            ...
            onPressed: () {
              NetworkService.allowElevationPromptAgain();
              // A failed download is one of the errors this panel shows, and
              // the install routes rethrow it as long as it is set. The other
              // retries clear it; without this one the retry failed again
              // within a second, with the same message, every time.
              _restartFailedDownloads();
              setState(() {
                _artifactError = null;
  ...
  Future<void> _fallBackToFullImage() async {
    ...
    setState(() {
      ...
      _expectMinimalMdb = false;
      _isProcessing = true;
      // The background stage and the dashboard upload are a different run's
      // work now. The image write erases /data, so anything already uploaded
      // is gone, and a stale verdict from the stage would route straight
      // back to the panel the user just left.
      _mdbStageStarted = false;
      _mdbStageDone = false;
      _mdbStageError = null;
      _mdbStageProgress = 0;
    ...
    return PhaseLayout(
      ...
      actions: [
        if (_dbcUploadReady) ...[
          ...
          PhaseAction(
            label: _trampolineStartInFlight
                ? l10n.startingTrampoline
                : (mapsOnly ? l10n.dbcReadyButtonMaps : l10n.dbcReadyButton),
            icon: _trampolineStartInFlight ? Icons.hourglass_top : Icons.bolt,
            primary: true,
  ...
  Future<String?> _saveTrampolineFailureDiagnostics(
    TrampolineStartException error,
  ) async {
    ...
    try {
      ...
      debugPrint(
        'Trampoline: saved ${saved.length} diagnostic file(s) to '
        '${target.path}',
      );
      return target.path;
    } catch (e) {
  ...
  InstallTimeEstimate _autonomousHandoffEstimate() {
    final plan =
        _plan ??
        InstallPlan(
    ...
    return InstallTimeEstimate.forAutonomousHandoff(
      plan: plan,
      assets: InstallEstimateAssets(
        dbcStage0ImageBytes: _downloadState.imageFor(Board.dbc)?.expectedSize,
        dbcArtifactBytes: _downloadState.artifactFor(Board.dbc)?.expectedSize,
        osmTilesBytes: _downloadState
            .itemOfType(DownloadItemType.osmTiles)
            ?.expectedSize,
        routingTilesBytes: _downloadState
  ...
  Future<void> _startDbcVerification() async {
    ...
    if (generation == null) return;
    try {
      await _verifyDbcFlash(generation);
    } catch (e, stack) {
      debugPrint('UI: DBC verification failed: $e\n$stack');
      if (!_ownsReconnect(generation)) return;
      _reconnectAttempt.fail(generation, e);
      _failReconnect(l10n.errorPrefix(e.toString()));
    }
  }
  ...
  Future<void> _verifyDbcFlash(int generation) async {
    ...
    setState(() {
      ...
      _reconnectSubsteps = [
        ...
        Substep(id: 'completion', label: l10n.substepCheckCompletionRecord),
    ...
    try {
      ...
      _reconnectAttempt.fail(generation, e);
      _failReconnect(l10n.sshConnectionFailed(e.toString()));
      return;
    }
    if (!_ownsReconnect(generation)) return;
  ...
  Future<void> _startBluetoothPairing() async {
    ...
    Future<void> stopStaleStart() async {
      ...
      if (!_isDryRun) {
        try {
          ...
          debugPrint('UI: stale BLE start cleanup failed (ok): $e');
        }
      }
    }
    ...
    try {
      ...
      if (!_isDryRun && !_pairingVehicleStateChanged) {
        try {
          ...
          if (_stateBeforePairing != 'parked') {
            claimedVehicleState = _stateBeforePairing;
            claimedVehicleStateChange = true;
      ...
      if (isCurrent()) setState(() => _btAdvertisingSettling = false);
    } catch (e) {
      if (!isCurrent()) {
        await stopStaleStart();
        return;
      ...
      await _stopBluetoothPairing(advance: false);
      if (mounted) _setStatus(l10n.pairingStartFailed(e.toString()));
    }
  }
  ...
  Future<void> _onEnterKeycardSetup() async {
    ...
    //
    // is-enabled prints "masked" and exits 1, so it needs || true to be read
    // at all. The bootstrap image ships this unit masked.
    String enabledState = 'unknown';
    try {
      enabledState = (await _sshService.runCommand(
        'systemctl is-enabled librescoot-keycard 2>&1 || true',
      )).trim();
    } catch (_) {}
    if (enabledState == 'masked') {
      debugPrint('UI: unmasking librescoot-keycard for keycard setup');
      try {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      try {
        // is-active exits 3 while the unit is inactive, so without || true
        // the answer was an exception and the state stayed "unknown".
  ...
  Future<void> _keycardAddKnownCards(KeycardCapability capability) async {
    final uids = keycardsToAdd(
      ...
      dataKept:
          _plan?.mdb.action == BoardAction.upgrade ||
    ...
    try {
      for (var i = 0; i < 12; i++) {
        ...
        if (keycardReaderMissing(faults)) {
          debugPrint(
            'UI: keycard-service reports no reader (fault set: '
  ...
  Future<void> _stopActiveKeycardModes() async {
    _keycardLearningGeneration++;
    final masterOwner = _keycardMasterOwnerGeneration;
    if (_isDryRun) {
      _keycardLearning = false;
      _keycardLearningStarting = false;
      _keycardMasterLearning = false;
  ...
  Future<void> _startKeycardLearning() async {
    ...
    bool isCurrent() =>
        ...
        generation == _keycardLearningGeneration;
    Future<void> stopStaleStart() async {
      if (!isCurrent()) await _stopKeycardLearning(advance: false);
    }
  ...
  Future<void> _keycardStartMasterStage({bool retry = false}) async {
    ...
    _keycardMasterOwnerGeneration = generation;
    setState(() {
    ...
    Future<void> stopStaleStart() async {
      ...
      try {
        ...
        debugPrint('UI: could not stop stale master teach-in: $e');
      } finally {
        if (_keycardMasterOwnerGeneration == generation) {
          _keycardMasterLearning = false;
          _keycardMasterOwnerGeneration = null;
        }
      }
    ...
    try {
      ...
      final pendingStart = _sshService.redisLpush(
        'scooter:keycard',
        'learn:master:start',
      );
      _keycardMasterStartPending = pendingStart;
      try {
        await pendingStart;
      } finally {
        if (identical(_keycardMasterStartPending, pendingStart)) {
          _keycardMasterStartPending = null;
        }
      }
      if (!isCurrent()) {
        await stopStaleStart();
  ...
  void _handleKeycardEvent(String payload) {
    ...
    if (payload.startsWith('card-learned:')) {
      ...
      _keycardAdvanceTimer?.cancel();
      _keycardAdvanceTimer = Timer(
        const Duration(milliseconds: 1200),
        () async {
          if (!ownsKeycardMasterEvent(
            eventGeneration: generation,
            currentGeneration: _keycardLearningGeneration,
            ownerGeneration: _keycardMasterOwnerGeneration,
            mounted: mounted,
  ...
  Widget _buildKeycardSetup(AppLocalizations l10n) {
    return PhaseLayout(
      ...
      actions: switch (_keycardStage) {
        ...
        _KeycardStage.alreadyConfigured => _keycardAlreadyConfiguredActions(
          l10n,
        ),
        _ => const [],
      },
          ...
          switch (_keycardStage) {
            _KeycardStage.loading || _KeycardStage.done => Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text(
                  l10n.keycardPreparingReader,
                  style: TextStyle(color: Colors.grey.shade400),
                ),
              ],
            ),
            _KeycardStage.alreadyConfigured => _buildKeycardAlreadyConfigured(
              l10n,
            ),
  ...
  List<PhaseAction> _keycardAlreadyConfiguredActions(AppLocalizations l10n) => [
    ...
    PhaseAction(
      ...
      icon: Icons.nfc,
      onPressed: _canDriveKeycard && !_keycardLearningStarting
  ...
  List<PhaseAction> _keycardCardsActions(AppLocalizations l10n) {
    ...
    return [
      ...
      PhaseAction(
        ...
        onPressed:
            _canDriveKeycard && (_keycardLearning || !_keycardLearningStarting)
            ? (_keycardLearning
  ...
  /// Run the step's entry again after the reader did not answer: start the
  /// service, probe it, read the counts. The event subscription is kept; the
  /// entry does not open a second one.
  Future<void> _keycardRetryReader() async {
    if (_keycardStage == _KeycardStage.loading) return;
    debugPrint('UI: retrying the keycard reader');
    setState(() {
      _keycardCapability = null;
      _keycardStage = _KeycardStage.loading;
    });
  ...
  /// Start again on a fresh scooter by relaunching rather than resetting.
  /// A run leaves state in dozens of fields plus an open SSH session, and a
  /// missed one would carry the last scooter's answers into the next.
  ///
  /// Keep the cache because the next scooter needs the same artifacts.
  Future<void> _installAnother() async {
  ...
  Future<void> _finishAndExit({
    ...
    required bool keepDownloads,
  }) async {
    if (confirmed && !keepDownloads) await _offerCleanup();
    await _cleanupBeforeClose();
    exit(0);
  }
        ...
        actions: [
          PhaseAction(
            ...
            onPressed: () {
              ...
              Future.microtask(_startFinishEntry);
            },
          ),
        ],
        child: NoticeCard(
    ...
    return PhaseLayout(
      ...
      actions: [
        ...
        PhaseAction(
          ...
          primary: true,
          onPressed: () =>
              _finishAndExit(confirmed: confirmed, keepDownloads: false),
        ),
      ],
      child: Column(
        mainAxisSize: MainAxisSize.min,
  ...
  Widget _finishStatus(AppLocalizations l10n, FinalScreenState state) {
    ...
    final body = _dashboardTransferSkipped
        ...
        : switch (state) {
            ...
            FinalScreenState.completed => l10n.finishConfirmed,
          };
              ...
              children: [
                Text(
                  _dashboardTransferSkipped
                      ? l10n.finishSkippedHeading
                      : confirmed
                      ? l10n.finishCompleteHeading
                      : l10n.finishPendingHeading,
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 14,
                  ),
  ...
  Future<void> _cleanupMdb() async {
    ...
    String? status;
    try {
      status = await _sshService.runCommand(
